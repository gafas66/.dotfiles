...
This is moved to early-init.el file, so we can have org properly
installed before babel-loading this file.

* Installing packages

** Programming and Code completion

*** Company setup - most popular it seems
...
#+begin_src emacs-lisp
  ...
  (defun ek/company ()
    ...
    (use-package company
  	...
  	;;efine-key company-active-map (kbd "M-.") #'company-show-location)
  	;;efine-key company-active-map (kbd "RET") nil)
    	;;efine-key company-active-map (kbd "<backtab>")
  	;;(lambda ()
  	;;  (interactive)
...
*** Now make some kind of choice of completion
...
#+begin_src emacs-lisp
  ...
  (setq p-modes '(sh-mode-hook tcl-mode-hook ruby-mode-hook perl-mode-hook
  			     cperl-mode-hook emacs-lisp-mode-hook python-mode-hook))
  (mapcar (lambda (mode) (add-hook mode 'fic-mode)) p-modes)
  (setq fic-highlighted-words '("FIXME" "TODO" "NOTE" "KLUDGE" "BUG"))
  (set-face-attribute 'fic-face nil :foreground "red" :background "yellow")
#+end_src
...
#+begin_src emacs-lisp
  ...
  (setq my-re (regexp-opt '("MEDIA") 'words))
  (add-hook 'eat-mode
  	  (lambda ()
...
Show changes compared to git version in file
  ...
  (defun ek/helm ()
    (use-package helm
      ...
      (
       ...
       ("<f5> s"    . helm-find)
       ("S-<f4>"    . helm-execute-kmacro)
       ("M-s"       . helm-occur-visible-buffers)
       ("<f6> h"    . helm-emms)
       ("M-g a"     . helm-do-grep-ag)
       ("M-g l"     . goto-line)
       ("M-g g"     . helm-grep-do-git-grep)
       ("M-g M-g"   . helm-revert-next-error-last-buffer)
       ("M-g i"     . helm-gid)
       ))
...
*** Vertico for completion, avy for jump?, winum
...
#+begin_src emacs-lisp
  (defun ek/vertico ()
    ...
    ;;(use-package vertico-reverse :after vertico)
...
Show key options this is included from emacs version 30
...
#+begin_src emacs-lisp
  (use-package org :straight nil
    ...
    (setq org-default-notes-file "/home/kofoed/init/org/tasks.org")
...
#+begin_src emacs-lisp
  ...
  (setq org-capture-templates
        '(
  	("t" "General task"       entry (file+headline org-default-notes-file "Tasks")            "* TODO %?\nSCHEDULED: %t\n  %i\n  %a")
...
For presentations (to html also?)
...
#+begin_src emacs-lisp
  (use-package visual-fill-column
    :config
    (setq visual-fill-column 110
          visual-fill-column-center-text t))
#+end_src
*** Org drill

Apparently useful for training slides
...
Fix for missing meta key on debian 12
...
#+begin_src emacs-lisp
  (setq undo-outer-limit 1e12)

  (defun esk/new-scratch-buffer-in-org-mode ()
    (interactive)
    (switch-to-buffer (generate-new-buffer-name "*temp*"))
    (org-mode))
  (bind-key "<f7>" 'esk/new-scratch-buffer-in-org-mode)
#+end_src
...
* Setting up default windows and start server
