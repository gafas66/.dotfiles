        ...
        (if (window-system) nil
          (progn
            (if (getenv "XTERM_VERSION")
                ; xterm/uxterm on debian
                (progn
                  (when (string= (getenv "XTERM_VERSION") "XTerm(379)")
                    (global-set-key (kbd "ø") #'execute-extended-command)
                    (global-set-key (kbd "÷") #'kill-ring-save)
                    (global-set-key (kbd "ø") #'yank-pop)
                    (global-set-key (kbd "¼") #'beginning-of-buffer)
                    (global-set-key (kbd "¾") #'end-of-buffer)
                    (global-set-key (kbd "ä") #'kill-word)
                    (global-set-key (kbd "í") #'back-to-indentation)
                    (global-set-key (kbd "M-O Q") #'hydra-shell-stuff/body)
